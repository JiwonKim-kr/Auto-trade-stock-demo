...
- **레짐 필터(거시 = 예측이 아니라 대응)**: 거시·지정학 이벤트는 희소·비정상이라 알파(예측 시그널)로
  쓰지 않는다 — 시장 프록시(KODEX 200) 일간 σ 국면별로 신규 매수 노출 배수를 결정적으로 적용
  (CALM×1 · ELEVATED×0.5 · STRESS×0, **청산 경로는 축소 안 함**). LLM 프롬프트에도 같은 제약을
  인코딩하고 [시장 레짐]을 컨텍스트로 제공(판단은 LLM, 노출 축소는 시스템). 판정 실패는 UNKNOWN(×1)
  — 시장 오버레이 조회 실패로 전 매수를 막지 않는다(종목별 방어는 게이트·가드레일이 fail-closed).
...
- **결정적 청산(LLM 우회 — 구현됨)**: 포지션 손실 **−8%(손절)** 또는 보유 **20거래일 초과(타임스톱)**
  시 해당 심볼의 LLM 판단을 건너뛰고 강제 SELL 생성(`engine/exits.py`). 진입은 다층 방어인데 청산이
  LLM 재량뿐이면 깊은 손실이 HOLD 관성에 방치된다는 갭의 해소. 타임스톱은 달력일이 아니라
  **거래일**(자산곡선 날짜 수) 기준, 추가매수에도 최초 진입 시각 유지. 시세 조회 실패 시 손절 판정
  보류(허위 청산 방지). LIVE 는 DB 필수(미설정 시 DRY_RUN 강제 강등 — 일일 한도·리컨실이 DB 전제).
...
- **리컨실(구현됨)**: **매 틱, 틱보다 먼저** 기대 포지션(직전 스냅샷 + 이후 SUBMITTED 순증감) ↔ 토스
  `holdings` 대조(`orders/reconcile.py` 순수 비교 + `position_snapshots`/`positions` 테이블).
  불일치 → 감사 기록, **LIVE 는 킬스위치 자동 발동**(DRY_RUN 은 기록만 — 수동 매매가 정상인 관찰 단계).
