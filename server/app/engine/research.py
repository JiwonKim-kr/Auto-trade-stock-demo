...
조사 모델은 판단 모델(Opus 4.8)과 분리 — 검색·요약 작업엔 Sonnet 이 충분하고 저렴
(2026-07-11 비용 절감 세트). 클라이언트 주입형이라 API 키 없이 테스트 가능.
"""
...
class ResearchConfig:
    model: str = "claude-sonnet-5"   # 조사용 — 검색+요약엔 충분·저렴(판단은 Opus. 2026-07-11 비용)
    max_tokens: int = 2000
    max_searches: int = 2            # web_search max_uses — 뉴스 브리프엔 1~2회로 충분(2026-07-11 비용)
