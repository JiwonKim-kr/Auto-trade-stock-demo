...
- BUY: 한도들의 최소값(1주문 한도 · 현금매수여력 · 종목당 비중 여유)을 ceiling으로,
       `ceiling × confidence`를 목표 금액으로 잡아 정수 주(KR) 수량 계산. 1주 미만이면 주문 안 함.
- SELL: 보유 전량 청산(보유 없으면 주문 없음).
- HOLD: 주문 없음.
...
def _buy_quantity(ctx: CandidateContext, decision: Decision, cfg: GuardrailConfig,
    ...
    if ceiling <= 0:
        return Decimal(0)
    target = ceiling * Decimal(str(decision.confidence))
...
def allocate(decision: Decision, ctx: CandidateContext, config: GuardrailConfig,
             *, client_order_id: str | None = None) -> OrderRequest | None:
    ...
    # BUY
    qty = _buy_quantity(ctx, decision, config, price)
    if qty < 1:
        return None
