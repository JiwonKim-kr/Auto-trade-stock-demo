...
> · **리컨실**(포지션 대조 — LIVE 불일치 시 자동 거래 중단). **183개 테스트 통과**, **실계좌 DRY_RUN
> end-to-end 검증(실주문 0)**. 데스크톱 앱·GCP 인프라·LIVE 전환은 [로드맵](#로드맵) 참조.
...
# 4) 테스트
.venv/Scripts/python -m pytest -q                   # 183 passed
