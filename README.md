...
> Sharpe/MDD/표본 게이트) · **로컬 상시 운용**(내장 틱 루프·유니버스 로테이션·LLM 비용가드·틱 직렬화).
> **263개 테스트 통과**, **실계좌 DRY_RUN end-to-end 검증(실주문 0)**. 데스크톱 앱·GCP 인프라·LIVE
> 전환은 [로드맵](#로드맵) 참조.
...
└──────────────────────┘       JSON             │   ├─ 주문층(모드게이트+하드가드레일)     │
   ...
   └──────────────────┘                                  │
       ...
       토스·AI 키         주문/포지션/감사    openapi.toss…    claude-opus-4-8    KOSPI+KOSDAQ
```
...
KRX 시드(KOSPI+KOSDAQ) ∪ 워치리스트 ∪ 보유 종목   ← 후보 상한으로 캔들 호출 수 제어(레이트리밋)
  ...
  → LLM 판단 (claude-opus-4-8: BUY/SELL/HOLD + confidence)       ← 사이징은 안 함
...
- **결정적 청산(LLM 우회)** — 포지션 손실 −8%(손절) 또는 보유 20거래일 초과(타임스톱) 시 **LLM 판단을 거치지 않고 강제 매도**. 깊은 손실이 LLM 의 HOLD 관성에 방치되는 것을 차단.
- **LIVE 는 DB 필수** — `DATABASE_URL` 없이 LIVE 를 켜면 DRY_RUN 으로 **강제 강등**(일일 한도 누적·리컨실·멱등 2차방어가 DB 전제).
...
# 4) 테스트
.venv/Scripts/python -m pytest -q                   # 263 passed

# 5) 서버 (DRY_RUN, 토스 엔드포인트는 creds 필요)
.venv/Scripts/python -m uvicorn app.main:app --reload
```

`/internal/tick` 가 전 파이프라인을 돈다(Anthropic 키 있으면 Opus 4.8 + web_search, 없으면 결정적 폴백 판단기).

**로컬 상시 운용(페이퍼 트레이딩)** — `scripts/.env` 에 아래를 넣고 **런처**로 켜두면 장중 자동 틱이 돈다
(서버는 process env 만 읽으므로 `.env` 로드는 런처가 담당 — 상세는 [USAGE.md](USAGE.md)):

```bash
# scripts/.env 에 추가
DATABASE_URL=sqlite+aiosqlite:///./trading.db   # 영속화(페이퍼 장부·엔진 상태·로테이션)
SYMBOL_SOURCE_PATH=data/krx_symbols.json        # 전 종목 유니버스(틱마다 코호트 순환)
TICK_INTERVAL_SEC=300                           # 5분 간격, KST 평일 09:00–15:30 에만 실행
ANTHROPIC_API_KEY=sk-ant-...                    # LLM 판단(없으면 결정적 폴백으로 관찰만)
```

```powershell
server/.venv/Scripts/python server/scripts/run_local.py    # 런처 — 켜두면 자동
```

성과는 `GET /api/evaluation` 으로 확인(완결 트레이드 N≥100 쌓일 때까지 "판단 보류"가 정상).
...
| `TICK_INTERVAL_SEC` | 내장 틱 루프(0=비활성 — 운영은 Cloud Scheduler) | 0 |
| `JUDGE_TOP_N` · `DAILY_LLM_DECISION_CAP` | LLM 비용가드(틱당 매수후보 상한 · 일일 판단 상한→폴백 강등) | 10 · 400 |
| `CANDLE_CACHE_TTL_MINUTES` | 캔들 TTL 캐시(429 방어, 0=비활성) | 60 |
| `NOTIFY_TELEGRAM_BOT_TOKEN` · `NOTIFY_TELEGRAM_CHAT_ID` | 알림(서킷브레이커·리컨실·틱 실패·킬스위치) | 없음(무음) |
...
| `ADV_POOL_SIZE` · `UNIVERSE_EXPLORE_RATIO` | 유니버스 2단계 선정(ADV 상위 풀 · 탐색 비율) | 300 · 0.2 |
...
| `EXIT_STOP_LOSS_RATE` · `EXIT_TIME_STOP_DAYS` · `EXIT_RULES_ENABLED` | 결정적 청산(손절·타임스톱) | 0.08 · 20 · true |
...
- **AI:** 판단 `claude-opus-4-8`(effort medium) · 조사 `claude-sonnet-5` + `web_search`(24h 캐시) — Fable 5 는 **비용 부담으로 전환**(2026-07, refusal 폴백 메커니즘은 유지·기본 비활성). 구조화 출력은 `output_config`.
...
- [USAGE.md](USAGE.md) — **환경설정 · 사용 가이드**(설치→자격증명→상시 운용→모니터링→문제해결)
- [IMPLEMENTATION-PLAN.md](IMPLEMENTATION-PLAN.md) — **구현·개선 계획서**(P0 안전 필수 → 성능 → M2/M3, 상세 스펙)
- [SELF-EVALUATION.md](SELF-EVALUATION.md) — **자기 평가 기록**(안전성 실측·효용성·준비도, 2026-07)
