...
> LLM 판단 → 결정적 사이징 → DRY_RUN 주문. **KRX 심볼 소스 연동**(KOSPI+KOSDAQ 시드) · **손실
> 서킷브레이커**. **128개 테스트 통과**, **실계좌 DRY_RUN end-to-end 검증(실주문 0)**. 데스크톱 앱·GCP
> 인프라·DB 영속화·LIVE 전환은 [로드맵](#로드맵) 참조.
...
- **멱등** — `clientOrderId` (재시도 중복주문 방지, DB UNIQUE 예정).
