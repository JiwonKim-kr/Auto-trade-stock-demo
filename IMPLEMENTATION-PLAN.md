...
| 티커 매핑 | **쿼리 규칙**: 종목명으로 검색 + 제목·요약에 종목명 포함 검사(`mapping_method="naver_query+name_match"`). 시황 기사는 매핑 실패로 자동 제외 | 고유번호→티커(공짜) |
| 카테고리 | — | "공시" 공짜 |

- 쿼터: 25,000건/일 — 200종목 × 30분 간격 폴링(08:00–18:00) ≈ 4,200콜/일로 여유.
- 동명 오탐(지주사·유사명)은 name_match 로 1차 방어 — 잔여는 분석 단계 수동 점검 항목.
