...
Cloud Scheduler(잡 2개, OIDC) ──POST──▶ Cloud Run(request-based, min=0/max=1) ──▶ Supabase PG(세션 풀러)
  ① tick:   */20 9-15 * * 1-5 (Asia/Seoul) → /internal/tick                       (AWS 서울 — §아래 DB 결정)
...
| 항목 | 산정 근거 | 월 예상 |
|---|---|---|
| Cloud Run | 1 vCPU/1GiB request-based. 틱 ~420회/월(20분 간격) × 1–8분(LLM 대기 포함 과금) − 무료구간 | **$0–5** |
...
| **GCP 소계** | (Cloud SQL 전환 시 +$11–13) | **≈ $0–6** |
| Anthropic 판단 (Opus·effort medium·프롬프트 캐시) | 실질 40–100콜/일(20분 틱) | **$8–30** |
| Anthropic 조사 (Sonnet + web_search) | §3.10 캐시 + 상위 3 + 검색 ≤2회 (원래 최대 390콜/일 $80–300 이던 지배 항목) | **$3–12** |
| **총계** | | **≈ $12–50/월** |

- **비용 절감 세트(2026-07-11 적용 — 사용자 결정 "테스트 단계 검증 후 되돌릴 수 있는 건 지금 다")**:
  ① 조사 `max_searches` 4→2 ② `RESEARCH_TOP_N` 5→3 ③ 틱 5→**20분**(페이퍼 개시 시점 결정
  2026-07-11 — 강제청산 감지 지연 최대 20분이 대가, 표본 축적 후 `*/10`·`*/5` 복귀 검토)
  ④ 판단 `effort` high→medium(**테스트 단계에서
...
| `google_cloud_scheduler_job` ×2 | ① tick `*/20 9-15 * * 1-5`(비용 — 표본 축적 후 `*/10`·`*/5` 검토) ② report `30 16 * * *` — 둘 다 **`time_zone="Asia/Seoul"`**(UTC 환산 불필요·오프바이원 예방), HTTP POST + **OIDC token**(scheduler-sa, audience=run_url) | **`attempt_deadline="900s"`**(기본 3분이면 틱 도중 잘림)·`retry_count=0`(다음 파이어가 커버 — 재시도는 중복 파이어만 만든다, 락이 직렬화하지만 무의미). 15:30 초과분·휴장일은 서버가 거른다(§3.6) |
...
## §8. 논문 데이터 수집 — 감성 F1 ↔ 알파 상관 연구 (2026-07-11 착수)

> **연구 주제**: 감성 분류 F1 이 높은 모델이 반드시 알파(IC)가 높지 않음을 실측.
> ChronoBERT 계열 관찰(하위 예측 모델이 언어 이해 한계를 보완)과의 대조가 목표.
> **핵심 원칙 = 전향(forward) 수집**: 수집시각 품질 지표·수정기사 최초버전 고정·
> LLM 라벨링 look-ahead 오염 회피(§7.1-1 과 동일 원리)는 전향으로만 성립. 소급분을 섞으면
> 반드시 컬럼으로 구분(`collected_at` ≫ `published_at` 인 데이터가 소급분).

### 8.1 스키마 (DB 3테이블 — 거래 테이블과 분리)

| 테이블 | 필드 | 비고 |
|---|---|---|
| `news` | id PK · symbol · headline · press · url · published_at(tz) · collected_at(tz) · source · mapping_method · category? · cluster_id? · body? | **관측 단위 = (url, symbol) 쌍**(UNIQUE) — 한 기사가 2종목이면 2행(이벤트 스터디 관행). cluster_id 는 분석 단계에서 채움(원본 전부 보존) |
| `news_labels` | id · news_id FK · label · label_version · labeled_at | 골드 라벨. 재라벨링(자기일치도)은 label_version 으로 구분 |
| `news_model_outputs` | id · news_id FK · model · model_version? · prompt_version · raw_output · parsed_label? · inferred_at | API 모델은 버전이 바뀜 — 실험 시점 기록 필수. 파싱 실패도 raw 보존 |

- 시각은 전부 **tz-aware UTC 저장**(repo `_utc()` 관행) — 표기·정렬은 KST 변환. 문서화: "KST 고정".
- `published_at` 출처 = **뉴스 검색 API `pubDate`**(RFC822, 분 단위) — 언론사 표기 시각 아님을 명기.
- 수정 기사: 전향 수집은 최초 관측 버전이 고정됨(UNIQUE 로 재수집 무시). 소급분은 수집 시점 버전.
- **소스 플랫폼(2026-07 확정)**: developers.naver.com 검색 API 가 **NCP NAVER API HUB** 로 이관 →
  본선은 API HUB(`naverapihub.apigw.ntruss.com/search/v1/news`, 헤더 `X-NCP-APIGW-API-KEY-*`).
  파라미터·응답 스키마는 developers.naver.com 과 동일 — `source="naver_api_hub"` 로 provenance 기록.
  한시적 무료(유료 전환 시 사전 공지). 지도 API 선례처럼 개발자센터 API 는 최종 하드 컷오프 예상 →
  전향 수집(12–24개월) 중 공백 방지 위해 처음부터 API HUB 채택.

### 8.2 소스·필드 매핑

| 필드 | NAVER API HUB(뉴스 검색) — 1차 소스 | DART OpenAPI(공시) — 2차(후속) |
|---|---|---|
| 헤드라인 | `title` — **복원 규칙**: 검색 하이라이트 `<b></b>` 등 태그 제거 + HTML 엔티티 unescape(이것이 원문) | `report_nm` |
| 보도 시각 | `pubDate`(분 단위, +0900) | 접수시각(분 단위, 정확) |
| 언론사 | `originallink` 도메인 | — (공시) |
| URL/중복 키 | `originallink`(없으면 `link`) | 접수번호 |
| 티커 매핑 | **쿼리 규칙 + 매핑 강도 구분(옵션 C)**: 종목명 검색 후 — 제목에 종목명=`title_match`(강), 요약에만=`desc_match`(약, 광범위 언급). 둘 다 저장(원본 보존), `mapping_method` 로 구분 → 분석에서 "제목만" vs "제목+요약" 로버스트니스. 어디에도 없으면 매핑 실패로 자동 제외(시황 기사) | 고유번호→티커(공짜) |
| 카테고리 | — | "공시" 공짜 |

- 쿼터: 25,000건/일 — 200종목 × 30분 간격 폴링(08:00–18:00) ≈ 4,200콜/일로 여유.
- 동명 오탐(지주사·유사명)은 매핑 강도(title/desc)로 1차 구분 — 잔여는 분석 단계 수동 점검 항목.
  광범위 시황·정치 기사가 본문에 종목명을 스치는 경우가 `desc_match` 로 격리되므로, 기본 분석은
  `title_match` 만으로 시작하고 `desc_match` 포함 여부를 로버스트니스 축으로 쓴다.

### 8.3 t+1 정렬 규칙 (news/alignment.py — 순수 함수, 논문 방법론에 그대로 전사)

```
return_window(published_kst, holidays) →
  d = published_kst 의 KST 날짜
  if 거래일(d) and 시각 < 15:30 → (진입=d 종가,            청산=next_td(d) 종가)   # "당일 종가→t+1"
  else                        → (진입=next_td(d) 시가,     청산=next_td(next_td(d)) 시가)  # "t+1 시가→t+2"
  (next_td = 다음 거래일 — 주말·공휴일 §3.6 캘린더. 금요일 15:30 후 보도 → 월요일 시가→화요일 시가)
```
수익률은 시장 조정(종목 소속 시장의 코스피/코스닥 지수 동일 구간 수익률 차감). 지수·개별 OHLCV 는
pykrx/FDR(수정주가 — **액면분할 종목으로 실측 검증 후 채택**, `fetch_history.py` 확장).

### 8.4 종목 선정·규모·운영

- 유니버스: **시총 상위 ∩ ADV20 상위(`symbol_stats` 재사용) ∩ 뉴스 빈도**로 50–200개 커트.
  타깃 목록은 `server/data/news_targets.json`(git 추적 — 스냅샷 버전 고정). 종목명은 KRX 시드에서 해석.
- 기간 12–24개월(하락 구간 포함 유리) · 목표 5,000–10,000건(중복 제거 후).
- 운영: Scheduler 잡 ③ → `POST /internal/news/collect`(30분 간격 08:00–18:00) — **거래 틱과 분리**
  (레이트리밋·장애 격리). 야간 보도는 다음날 08:00 수집 — `collected_at−published_at` 랙에 주기 패턴이
  생김을 방법론에 명시(품질 지표 해석 시 참고).
- 시크릿 +2: `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`(NCP NAVER API HUB Application 의
  Client ID/Secret — 콘솔: All Services > Application Services > NAVER API HUB > Application 등록 >
  뉴스 검색 선택 > [인증 정보]). 시크릿 이름은 그대로 두고 값만 NCP 것으로(Terraform 변경 없음).

### 8.5 한계 문구 초안 (논문 방법론용)

1. 상폐·거래정지: KRX 시드는 현존 종목 — 생존편향 존재. "익일 가격 없음 → 관측 제외·건수 보고" 규칙.
2. 소급분(있다면): 헤드라인은 수집 시점 버전(최초 버전 보장 없음) + LLM 라벨 look-ahead 오염 가능 —
   전향 구간과 분리 보고.
3. `published_at` 은 네이버 API 기준 — 언론사 자체 표기와 다를 수 있음(±분 단위).
4. 감성 라벨의 골드는 수동 라벨(자기일치도 label_version 재라벨로 측정).
5. **매핑 정밀도(2026-07-21 첫 수집 실측, 12종목 2,104건)**: `desc_match` 86% · `title_match` 14%.
   NAVER `description` 은 검색어를 포함한 스니펫이라 요약 매핑은 거의 항상 성립(정밀도 낮음 —
