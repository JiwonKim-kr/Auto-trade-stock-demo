...
**시크릿 버전 주입(TF 밖 — state 에 비밀 금지).** 6종 전부 없으면 Cloud Run 배포가 실패한다:

```powershell
foreach ($s in "TOSS_CLIENT_ID","TOSS_CLIENT_SECRET","ANTHROPIC_API_KEY","API_KEY","DATABASE_URL","NOTIFY_TELEGRAM_BOT_TOKEN") {
...
통과 후: `terraform apply -var scheduler_paused=false`
