...
PC 전원과 무관하게 거래 파이프라인을 계속 돌려 관측한다. **운영과 완전 분리**:

- 별도 Cloud Run 서비스(`toss-trader-sandbox`) + 전용 Scheduler 잡(10분 간격, 24/7)
- **토스·Anthropic·텔레그램 시크릿을 주지 않는다** → 외부 호출 0·LLM 비용 0
- DB 는 같은 Supabase 지만 **`sandbox` 스키마**로 격리(`DB_SCHEMA=sandbox`).
  search_path 를 그 스키마로 고정하므로 **운영 테이블(public)이 보이지도 않는다** —
  스키마 없이 PG 로 샌드박스를 켜면 앱이 **기동을 거부**한다(오염 방지).

```powershell
terraform apply -var enable_sandbox=true -var sandbox_paused=false
terraform output sandbox_url
```

비용: Cloud Run min=0 · 10분 간격 · 틱당 수 초 → 월 $1~5 수준(무료 구간과 겹침).

**보는 법**: 샌드박스도 IAM 잠금이라 브라우저 직결이 안 된다. 로컬 대시보드를 샌드박스
스키마에 붙여서 본다(§USAGE 6):

```powershell
$env:DATABASE_URL="<운영과 같은 Supabase URL>"; $env:DB_SCHEMA="sandbox"
server/.venv/Scripts/python server/scripts/run_local.py --port 8000
#   → http://127.0.0.1:8000/dashboard  (샌드박스 스키마의 틱·주문·페이퍼가 보인다)
```

끄기: `terraform apply -var enable_sandbox=true -var sandbox_paused=true` (잡만 정지) 또는
`-var enable_sandbox=false` (서비스·잡 삭제 — `sandbox` 스키마 데이터는 남는다).
